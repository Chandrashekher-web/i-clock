<?php
    ...
    class Commands_history_model extends My_Model
    {
        ...
        public function get_all_history_commands($pagingParams = array(), $conditions_arr = array(), $table_name = null)
        {
//            p($table_name);
            $where_condition = '';
            ...
            if (!empty($table_name) && $table_name == 'unsuccessful')
            {
                $this->db->from('reader_command_history_unsuccessful as rc');
            }
            else
            {
                $this->db->from('reader_command_history as rc');
            }
            ...
            if (!empty($conditions_arr))
            {
                if (!empty($conditions_arr['commands_filter']))
                {
                    foreach ($conditions_arr['commands_filter'] as $key => $filter_field)
                    {
                        ...
                        else if ($field_name == 'date_from')
                        {
                            $field_name = "DATE_FORMAT(rc.created_at,'%Y-%m-%d')";
                            //p($date);
                            $new_date = date("Y-m-d", strtotime(str_replace('/', '-', $date_from)));
                            $string = " >= '" . $new_date . "'";
                        }
                        else if ($field_name == 'date_to')
                        {
                            $field_name = "DATE_FORMAT(rc.created_at,'%Y-%m-%d')";
                            //p($date);
                            $new_date = date("Y-m-d", strtotime(str_replace('/', '-', $date_to)));
                            $string = " <= '" . $new_date . "'";
                        }
