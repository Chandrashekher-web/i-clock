<?php
    ...
    class Employee extends MY_Controller
    {
        ...
        public function add_employee($employee_id = null, $page = null)
        {
            ...
            {
                $this->form_validation->set_rules('pin', 'Pin', 'required|trim|max_length[9]|unique[employee.pin.site_id.' . $session_site_id . '.employee_id]');
            }
            else
            {
                $this->form_validation->set_rules('pin', 'Pin', 'required|trim|max_length[9]|unique[employee.pin.site_id.' . $session_site_id . ']');
            }
            $dataArray = array();
            $check_user_access = $this->Site_model->check_user_access_by_site_id($session_site_id);
            ...
            $access_groups_arr = array();
            $access_groups = $this->Reader_access_groups_model->get_reader_access_groups_array($session_site_id);
            if (!empty($access_groups))
            {
                $access_groups_arr = $access_groups;
            }

            $dataArray['arr_access_groups'] = add_blank_option($access_groups_arr, "-Select Group-");

            if ($this->form_validation->run() == false)
            {
                $dataArray['form_caption'] = "Add Employee";
                $dataArray['form_action'] = current_url();
                $dataArray['user_access'] = $check_user_access;
               
                $dataArray['start_date'] = date('Y-m-d');
                $dataArray['end_date'] = date('Y-m-d');
                if (!empty($employee_id))
                {
                    $dataArray['form_caption'] = 'Edit Employee';
                    $employee_data = $this->Employee_model->get_employee_by_id($employee_id);
                    $dataArray['arr_sub_category'] = $this->Sub_category_model->get_sub_category_array_by_category_id($employee_data['category_id']);
                    ...
                    $dataArray['category_id'] = $employee_data['category_id'];
                    $dataArray['sub_category_id'] = $employee_data['sub_category_id'];
                    $dataArray['address'] = $employee_data['address'];
                    $dataArray['property_number'] = $employee_data['property_number'];
                    $dataArray['phone'] = $employee_data['phone'];
                    $dataArray['vehicle_reg_no'] = $employee_data['vehicle_reg_no'];
                    $dataArray['id_no'] = $employee_data['id_no'];
                    $dataArray['permanent_user'] = $employee_data['permanent_user'];
                    $dataArray['username'] = $employee_data['username'];
                    $dataArray['access_code_generator'] = $employee_data['access_code_generator'];
                    $dataArray['administrator'] = $employee_data['priv'] == ADMIN_USER_PRIV ? "Yes" : "No";
                    $dataArray['access_group'] = $employee_data['access_group'];
                    $dataArray['employee_id'] = $employee_id;
                    $dataArray['start_date'] = $employee_data['start_date'];
                    $dataArray['end_date'] = $employee_data['end_date'];
            ...
            else
            {
                ...
                {
                    $start_date = getSADate($this->input->post('start_date'));
                }

                if (!empty($this->input->post('end_date')))
                {
                    $end_date = getSADate($this->input->post('end_date'));
                }
                ...
                if (!empty($start_date) && !empty($end_date))
                {
                    ...
                    {
                        $status = "Inactive";
                        $employee_reader = $this->iclock_model->get_employee_reader($employee_id);
                        if (!empty($employee_reader))
                        {
                            $pin = $this->input->post('pin');
                            foreach ($employee_reader as $reader)
                            {
                                ...
                                $command = "DATA DEL_USER PIN=" . $pin;
                                $delete_reader = $this->iclock_model->delete_employee_reader($dataValues1);
                                ...
                                $save_command = $this->iclock_model->save_command($dataValues2);
                            }
                        }
                    }
                }
                ...
                $dataValues = array(
                    ...
                    'permanent_user' => $this->input->post('permanent_user'),
                    'username' => $this->input->post('username'),
                    'access_code_generator' => $this->input->post('access_code_generator'),
...
//            p($resultdata['resultSet']);
        ...
        public function show_total_employee_report($key)
        {
            ...
            if ($this->form_validation->run() == false)
            {
                ...
                {
                    $dataArray['form_caption'] = "Employees with “0” fps Report";
                }
                else if ($key == 'with_0_rfcard')
                {
                    $dataArray['form_caption'] = "Employees with “0” RF card Report";
                }
                ...
                else if ($key == 'remote_reader_unlock')
                {
                    $dataArray['form_caption'] = "Remote Reader Unlock";
                }

                $dataArray['form_action'] = current_url();
                $dataArray['employee_filter'] = get_custom_config_item('employee_filter');
...
//            p($_POST);
            ...
            $employee_filter = $this->input->post('employee_filter');
            $key = $this->input->post('key');
            if ($key == 'total')
            {
                $empdata = $this->Employee_model->get_total_employees($site_id, $employee_filter);
            }
            else if ($key == 'with_0_fps')
            {
                $empdata = $this->Employee_model->get_total_employees_with_0_fps($site_id, $employee_filter);
            }
            else if ($key == 'with_0_rfcard')
            {
                $empdata = $this->Employee_model->get_total_employees_with_0_rfcard($site_id, $employee_filter);
            }
            else if ($key == 'per_reader')
            {
                $empdata = $this->Employee_model->get_total_employees_per_reader($reader_id, $employee_filter);
            ...
            {
                $empdata = $this->Reader_model->remote_reader_unlock($site_id);
            }
            $count = count($empdata);
            ...
            else if ($key == 'total')
            {
                ...
                if (!empty($empdata))
                {
                    $output .= "<table id='myTable' class='table table-striped table-bordered'><tr><th>Pin No</th><th>Name</th><th>Password</th><th>RF Card</th><th>FP Count</th><th>Face Count</th></tr>";
                    foreach ($empdata as $key => $employee)
                    {
            ...
            {
                $output = '<p>Total records count : ' . $count . '</p>';

                if (!empty($empdata))
                {
                    $sno = 1;
                    $output .= "<table id='myTable' class='table table-striped table-bordered'><tr><th>Login Name</th><th>Reader Name</th><th>SN</th><th>Command</th><th>Created Date</th></tr>";
                    foreach ($empdata as $key => $employee)
                    {
                        $output .= "<tr><td>" . $employee['login_user'] . "</td>";
                        $output .= "<td>" . $employee['reader_name'] . "</td>";
                        $output .= "<td>" . $employee['r_sno'] . "</td>";
                        $output .= "<td>" . $employee['command'] . "</td>";
                        $output .= "<td>" . get_date_d_m_y_time($employee['created_at']) . "</td>";
                    }
                }
                else
                {
            ...
            else
            {
                ...
                if (!empty($empdata))
                {
                    $output .= "<table id='myTable' class='table table-striped table-bordered'><tr><th>Pin No</th><th>Name</th><th>Password</th><th>RF Card</th></tr>";
                    foreach ($empdata as $key => $employee)
                    {
                        $output .= "<tr><td>" . $employee['pin'] . "</td>";
                        $output .= "<td>" . $employee['name'] . "</td>";
                            ...
                            while (($data = fgetcsv($handle, 1000, ",")) !== FALSE)
                            {
                                if ($row > 1)
                                {
                                    ...
                                    if (!empty($data))
                                    {
                                        ...
                                        $Category = $this->Category_model->get_category_by_name(trim($data[3]));
                                        $SubCategory = $this->Sub_category_model->get_sub_category_by_name(trim($data[4]));
                                        ...
                                        $address = trim($data[5]);
                                        $property_number = trim($data[6]);
                                        $phone = trim($data[7]);
                                        $vehiclereg = trim($data[8]);
                                        $ID = trim($data[9]);
                                        $permanentuser = trim($data[10]);
                                        $username = trim($data[11]);
                                        $password = trim($data[12]);
                                        $accesscodegenerator = trim($data[13]);
                                        $access_group = trim($data[14]);
                                        ...
                                        $dataValue = array(
                                            ...
                                            'id_no' => $ID,
                                            'permanent_user' => (!empty($permanentuser)) ? 'Yes' : 'No',
                                            "username" => $username,
                                            "password" => $password,
                                            "access_code_generator" => (!empty($accesscodegenerator)) ? 'Enable' : 'Disable',
                                            'visit_multi_times' => 'Yes',
                                            "status" => 'Active',
...
//            p($selectchk);
        ...
        public function get_employee_with_duplicate_fps()
        {
            ...
            {
                $count2 = 1;
                $output .= "<table id='myTable' class='table table-striped table-bordered'><tr><th>S.No.</th><th>Employee Info</th>";
                if ($check_admin_type == 'Super Admin' || $check_admin_type == 'Site Admin')
                {
                    $output .= "<th>Delete FPs</th>";
                }
                $output .= " </tr>";
                foreach ($empdata as $key => $employee)
                {
                    ...
                    $output .= "<td>";
                    $output .= "<table id='myTable2' class='table table-striped table-bordered'><tr><th>Pin No</th><th>Name</th><th>Password</th><th>RF Card</th>";
                    if ($check_admin_type == 'Super Admin' || $check_admin_type == 'Site Admin')
                    {
                        $output .= "<th>Delete FP</th>";
                    }
                    $output .= " </tr>";
                    foreach ($employee as $value)
                    {
                        ...
                        if ($check_admin_type == 'Super Admin' || $check_admin_type == 'Site Admin')
                        {
                            $output .= "<td style='text-align:center;'><a href='" . base_url() . "admin/employee/delete_employee_fp/" . $value['id'] . "' onclick=\"return confirm('Are you sure you want to delete the fingerprint for this employee?');\" class='cancel-status'><i class='fa fa-trash-o action-icon'></i></a></td>";
                        }
                        $output .= "</tr>";
                        $ids .= $value['id'] . '|';
                    }
                    ...
                    if ($check_admin_type == 'Super Admin' || $check_admin_type == 'Site Admin')
                    {
                        $output .= "<td style='text-align:center;'><a href='" . base_url() . "admin/employee/delete_employee_fp/" . $ids . "' onclick=\"return confirm('Are you sure you want to delete this fingerprint for all the employees?');\" class='cancel-status'><i class='fa fa-trash-o action-icon'></i></a></td>";
                    }
                    $output .= "</tr>";
                    $count2 ++;
                }
